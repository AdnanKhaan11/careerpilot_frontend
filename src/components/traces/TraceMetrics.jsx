...
export default function TraceMetrics({ metrics }) {
  const cards = [
    ...
    {
      title: "Memory",
      value: getMemoryCount(metrics),
      subtitle: "Memory operations",
      icon: Database,
    },
    {
      title: "Tokens",
      value: getTokenCount(metrics).toLocaleString(),
      subtitle: "Consumed",
      icon: Coins,
    },
  ];
  ...
  return (
    <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
      {cards.map(({ icon: Icon, subtitle, title, value }) => (
        ...
        >
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--cp-text-muted)]">
              {title}
            </p>
            <p className="mt-2 text-2xl font-bold tracking-tight">{value}</p>
            <p className="mt-1 text-[11px] text-[var(--cp-text-muted)]">
              {subtitle}
            </p>
          </div>
...
function getMemoryCount(metrics) {
  return firstNumber([
    ...
    sum(
      metrics?.memory_lookups,
      metrics?.memory_writes,
      metrics?.memory_retrieved,
    ),
  ]);
}
...
function firstNumber(values) {
  ...
  return (
    numericValues.find((value) => Number(value) > 0) ?? numericValues[0] ?? 0
  );
}
